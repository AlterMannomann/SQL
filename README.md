...
Limitations are as well a technical and a philosophical problem and overall god may not have. See Limitations for more on this topic.
...
Basically, limitations would cause any "gad" to look for the possibility how to overcome this limitation. After several trials, the only way to overcome this limitation as a limited "god" that tries to create a simulation of a, whatsoever, universe I found until now, is to create more than just one universe. Whenever a universe reaches the point, where numbers are higher than the system limitation, it sounds like a good idea to start a new universe, with new fresh numbers within the limitation of the system. If the new "universe" starts with a new initialization value. Maybe one of the universes may develop something that we know as our universe. It is difficult to start thinking as a, whatsoever, *god". It implies responsability. Technical it is very easy. You have a system. It is limitated. You have to overcome this limitation. So why not create a system, that creates limited systems, endlessly. If you work with databases, you may know what I mean. It reduces the problems to costs and available space.

And, I never liked the idea of multiverses, it seems to be the only solution so far. You create limited systems on the top of limitated systems. Whenever you reach the limit, a new fresh system is created, with the same limitations. And the only problems you have are ressources, whatever they cost, in whatever form of life, not system design. Such a system is compatible with whatever ressources you have. It is compatible with all systems that at least provide some ressources. Thinking as a "god" or better said, a creator, implies responsibility and technically compatibility with all unexpected situations, you may encounter and which you only know, when it happens.

Thus, my stupid solution, is to create a new limited space, whenever tha current system would reach it's limits. Starting the new "universe" with the force (or negative force, not sure about that) that would have caused an overflow. A nice side-effect is that the system startet with energy = 1 would create systems with an initialization value != 1. Opening possiblities.
