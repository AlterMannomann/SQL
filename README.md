...
To make it a little bit more complicated, I also can define the rules, how this should be handled. Not on an exception base. Preferably with generic rules that are valid in any situation apart from seed in dimension n = 0.
...
- **Generic overflow behavior**
  ...
  - The base node of a universe (dimension 0 position 0) is the only node that can create a new dimension. New dimension nodes are limited to position 1 on every dimension axis.
  - If the parent node of a new universe is in dimension n > 0, it can't output any energy anymore to the universe it is located in. It can receive energy from the parent universe, but it will never distribute energy to this parent universe, as it is now part of a new universe located in an old universe. Close nodes may still emit energy to the parent universe. *Anybody mentioned black holes^^?*
- **Node overflow**
  - Base node overflow
    - Should not happen, as the value of the base node should sum up to zero or has a temporary overflow value <= 1.
  - Node (fully connected, no escape dimension available) overflow
    - Build a new universe with this node as a parent.
      - Only option as we are fully connected (no position escape) and have no more dimensions left to escape.
  - Node (fully connected, escape dimension available) overflow
    - Build a new dimension on the base node
      - Can't escape to positions as fully connected.
  - Node (not fully connected) overflow
    - Create a new position in the dimension not connected.
    - Number space for positions exhausted
      - Build a new dimension on the base node
    - Number space for dimensions exhausted
