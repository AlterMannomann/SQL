...
## Space point trees
There is only one universe seed possible, a space point that has **no** parent. As of the fact, that IT systems can't differentiate well between -0 and +0, what would have been my preferred solution, I'm using base coordinates 1, 2 (base) and -1, -2 (mirror) for the basic point trees. To keep things a little bit smaller, I limit the dimensions currently to n = 3. But I'm using a database, so it is only a question of available space and processing capacities.
