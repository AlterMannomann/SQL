...
BEGIN
  ...
  l_run_id := '010';
  BEGIN
    INSERT INTO usim_multiverse (usim_base_sign) VALUES (2) RETURNING usim_base_sign INTO l_sql_number_result;
    -- input should be prevented by constraint
    l_fail_message := l_test_object || ' - ' || l_test_section || ' - ' || l_run_id || ': [' || l_sql_number_result || '] insert 2 on usim_base_sign should not work.';
    usim_test.log_error(l_test_id, l_fail_message);
    l_tests_failed := l_tests_failed + 1;
  EXCEPTION
    WHEN OTHERS THEN
      IF INSTR(SQLERRM, 'USIM_SIGN_MLV_CHK') > 0
      THEN
        l_tests_success := l_tests_success + 1;
      ELSE
        l_fail_message := l_test_object || ' - ' || l_test_section || ' - ' || l_run_id || ': unexpected error ' || SQLCODE || ': ' || SQLERRM;
  ...
  l_run_id := '011';
  BEGIN
    INSERT INTO usim_multiverse (usim_mirror_sign) VALUES (2) RETURNING usim_mirror_sign INTO l_sql_number_result;
    -- input should be prevented by constraint
    l_fail_message := l_test_object || ' - ' || l_test_section || ' - ' || l_run_id || ': [' || l_sql_number_result || '] insert 2 on usim_mirror_sign should not work.';
    usim_test.log_error(l_test_id, l_fail_message);
    l_tests_failed := l_tests_failed + 1;
  EXCEPTION
    WHEN OTHERS THEN
  ...
  l_run_id := '014';
  BEGIN
    UPDATE usim_multiverse SET usim_base_sign = -1 RETURNING usim_base_sign INTO l_sql_number_result;
    IF l_sql_number_result = 1
    THEN
      l_tests_success := l_tests_success + 1;
    ELSE
      l_fail_message := l_test_object || ' - ' || l_test_section || ' - ' || l_run_id || ': [' || l_sql_number_result || '] update -1 on usim_base_sign should not work.';
      usim_test.log_error(l_test_id, l_fail_message);
      l_tests_failed := l_tests_failed + 1;
    END IF;
  EXCEPTION
  ...
  l_run_id := '015';
  BEGIN
    UPDATE usim_multiverse SET usim_mirror_sign = 1 RETURNING usim_mirror_sign INTO l_sql_number_result;
    IF l_sql_number_result = -1
    THEN
      l_tests_success := l_tests_success + 1;
    ELSE
      l_fail_message := l_test_object || ' - ' || l_test_section || ' - ' || l_run_id || ': [' || l_sql_number_result || '] update 1 on usim_mirror_sign should not work.';
      usim_test.log_error(l_test_id, l_fail_message);
      l_tests_failed := l_tests_failed + 1;
    END IF;
  EXCEPTION
