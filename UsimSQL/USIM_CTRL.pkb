CREATE OR REPLACE PACKAGE BODY usim_ctrl IS
  ...
  IS
    -- PRAGMA AUTONOMOUS_TRANSACTION; -- no effect on current constellation
    l_distance              NUMBER;
    l_usim_energy           NUMBER;
    l_usim_amplitude        NUMBER;
    l_usim_wavelength       NUMBER;
    l_usim_force            NUMBER;
    l_usim_target_sign      INTEGER;
    l_usim_source_sign      INTEGER;
    ...
    IS
      SELECT outp.usim_id_outp
       ...
       WHERE outp.usim_id_outp    = cp_usim_id_outp
             -- only records not processed
         AND outp.usim_processed  = 0
    ;
