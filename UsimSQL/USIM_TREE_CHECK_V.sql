...
CREATE OR REPLACE FORCE VIEW usim_tree_check_v AS
  SELECT usim_point_name
       ...
       , perfect_node_cnt
       , subtree_add + SUM(nodes_per_dimension) OVER (PARTITION BY usim_point_name ORDER BY usim_dimension ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cur_node_cnt
...
COMMENT ON COLUMN usim_tree_check_v.usim_dimension IS 'The dimension the nodes/points reside.';
